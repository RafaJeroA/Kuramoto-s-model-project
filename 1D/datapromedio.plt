...
set xlabel "(H [A/m])" # para la longitud  
set ylabel "(B [mT])" # para el cuadrado del periodo
...
# datos del ajuste.
f(x)=a+b*x
fit f(x) fichero using 1:2:3:4 xyerror via a,b
...
plot [-1:1001][-0:1.01] fichero index 0 title "R" w xyerror lc rgb "red" ps 0.5 pt 7, \
	fichero index 1 title "Valores núcleo laminado" w xyerror lc rgb "blue" ps 0.5 pt 7, \

# First plot
#plot 'data.txt' index 0 using 1:2:3 title "R" with xyerrorpoints lc rgb "red" ps 0.5 pt 7, \
#     '' index 0 using 1:2:3 title "Valores núcleo laminado" with xyerrorpoints lc rgb "blue" ps 0.5 pt 7
...
# plot [0:0.02][0:1.25] fichero u ($1):(log($2)) title "Valor Medido"  lc rgb "red" ps 1 pt 7
...
# línea continua sin puntos (dt 1) y anchura 3 (lw 3) 
